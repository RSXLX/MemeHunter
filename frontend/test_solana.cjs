...
async function main() {
    ...
    const PROGRAM_ID = new web3_js_1.PublicKey('BxH7tarDExkhkR44TdfhcH9kYviUXAj7jiQtVF2wdfDu');
    ...
    const user = web3_js_1.Keypair.fromSecretKey(secretKey);
    ...
    // Derive Session PDA
    const [sessionPda, bump] = web3_js_1.PublicKey.findProgramAddressSync([
        Buffer.from('session'),
    ...
    const authIx = new web3_js_1.TransactionInstruction({
        keys: [
            ...
            { pubkey: web3_js_1.SystemProgram.programId, isSigner: false, isWritable: false }, // System Program
        ],
        programId: PROGRAM_ID,
        data: authData,
    });
    try {
        const tx = new web3_js_1.Transaction().add(authIx);
        const txSig = await (0, web3_js_1.sendAndConfirmTransaction)(connection, tx, [user]);
        ...
        if (!accountInfo) {
            ...
            // Check Authority
            const onChainAuthority = new web3_js_1.PublicKey(accountInfo.data.slice(0, 32));
            const onChainSessionKey = new web3_js_1.PublicKey(accountInfo.data.slice(32, 64));
    ...
    const huntIx = new web3_js_1.TransactionInstruction({
        keys: [
    ...
    try {
        ...
        const tx = new web3_js_1.Transaction().add(huntIx);
        // Signers: SessionKey (for logic), User (for fees here)
        const txSig = await (0, web3_js_1.sendAndConfirmTransaction)(connection, tx, [user, sessionKey]);
