...
### 2026-01-27 - Hunt 逻辑后端化

| 变更 | 说明 |
|------|------|
| 删除 `routes/hunt.js` | HTTP hunt 路由废弃 |
| 删除 `services/txBroadcaster.js` | EVM 合约调用废弃 |
| 保留 WebSocket hunt | `gameSync.js` 处理所有 hunt 逻辑 |

**核心 Hunt 逻辑位置**: `websocket/gameSync.js` + `services/comboService.js`

**WebSocket Hunt 事件**:
- `hunt` - 客户端发起狩猎
- `huntResult` - 服务端返回结果（含连击/冷却）
